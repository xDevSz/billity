...
function Contracts() {
  ...
  const handleGeneratePdf = (event) => {
    ...
    // CORRIGIDO: Substituindo alert() por console.error
    if (!formData.agreed) { console.error('É necessário concordar com os termos.'); return; } 
    const selectedClient = clientsData.find(c => c.id === parseInt(formData.clientId));
    if (!selectedClient) { console.error('Por favor, selecione um cliente válido.'); return; } 
    ...
    try {
      ...
      doc.autoTable({ 
        ...
        body: [ 
            ...
            // CORREÇÃO: TROCA 'ao ${formData.periodUnit.slice(0, -1)}' por 'a.m.' ou 'a.a.' para consistência
            ['Taxa de Juros', `${formData.interestRate}% a.m. (${formData.interestType})`], // Corrigido para 'a.m.'
